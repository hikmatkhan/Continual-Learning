...
def _patched_parameters(
    ...
    if not self.track_higher_grads and time not in (-1, 0):
        raise ValueError(
            "The patched model is not tracking highers gradients. Only the "
...
class _MonkeyPatchBase(_abc.ABC, _torch.nn.Module):
    ...
    def init_fast_params(self):
        if not self.track_higher_grads:
            raise Exception(
                "Cannot get initial parameters when not tracking highers "
                "gradients."
            )
...
def monkeypatch(
    ...
    Args:
        ...
        track_higher_grads: if True, during unrolled optimization the graph be
            ...
            potentially tracking highers order gradients. This can be useful when
    ...
    Returns:
        ...
        of the optimizers in ``highers.optim``.
    """
