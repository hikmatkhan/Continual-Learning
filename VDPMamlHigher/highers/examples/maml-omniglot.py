...
"""
This example shows how to use highers to do Model Agnostic Meta Learning (MAML)
    ...
    for batch_idx in range(n_train_iter):
        ...
        for i in range(task_num):
            with higher.innerloop_ctx(
                ...
                # highers is able to automatically keep copies of
                # your network's parameters as they are being updated.
                spt_r_loss = 0
                for _ in range(n_inner_iter):
                    spt_logits = fnet(x_spt[i])
                    spt_loss = F.cross_entropy(spt_logits, y_spt[i])
                    spt_r_loss += spt_loss.detach().item()
                    diffopt.step(spt_loss)
                spt_mean_loss = spt_r_loss / n_inner_iter
        ...
        wandb.log({"M-trn_spt_loss:": spt_mean_loss,
                   "M-trn_qry_loss": qry_losses,
                   # "M-trn_spt_acc": mean_meta_train_spt_acc,
                   "M-trn_qry_acc": qry_accs,})
...
if __name__ == '__main__':
    ...
    wandb.init(project="MamlWithHigher", entity="hikmatkhan-")
    main()
