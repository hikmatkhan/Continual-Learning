...
class Net(nn.Module):
    def __init__(self, args):
        ...
        self.conv3d2 = VDP_Conv2D(64, 128, 3, weight_args=self.conv_init)
        self.relu = VDP_Relu()
        self.maxpool = VDP_Maxpool(2, 2)
        self.theflattening = VDP_Flatten()
        self.fullyCon1 = VDP_FullyConnected(128, 512, weight_args=self.fc_init)
        self.fullyCon2 = VDP_FullyConnected(512, 10, weight_args=self.fc_init)
    ...
    def forward(self, x_input):
        ...
        mu, sigma = self.conv3d2.forward(mu, sigma)
        mu, sigma = self.relu.forward(mu, sigma)
        mu, sigma = self.maxpool.forward(mu, sigma)
    ...
    def batch_loss(self, output_mean, output_sigma, label):
        ...
        loss_value = neg_log_likelihood + (self.tau * 1e-1 * self.conv2d1.kl_loss_term() +
                                           self.tau * 1e-2 * self.conv2d2.kl_loss_term() +
                                           self.tau * 1e-2 * self.conv3d2.kl_loss_term() +
                                           self.tau * 1e-3 * self.fullyCon1.kl_loss_term() +
                                           self.tau * 1e-4 * self.fullyCon2.kl_loss_term())
        return loss_value
