...
class VariationalPosterior(torch.nn.Module):
    def __init__(self, mu, rho, device):
        ...
        self.sigma = torch.log1p(torch.exp(self.rho)).to(self.devices)

    def sample(self):
        epsilon = self.normal.sample(self.rho.size()).to(self.devices)
        # reparametrizarion trick for sampling from posterior
        posterior_sample = (self.mu + self.sigma * epsilon).to(self.devices)
...
class Prior(torch.nn.Module):
    ...
    def __init__(self, args):
        ...
        self.s1 = torch.tensor([math.exp(-1. * self.sig1)], dtype=torch.float32, device=self.devices)
        self.s2 = torch.tensor([math.exp(-1. * self.sig2)], dtype=torch.float32, device=self.devices)
