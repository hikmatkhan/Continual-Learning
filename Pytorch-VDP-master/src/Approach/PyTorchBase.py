...
def run_sanity(args, model, train_loader, test_loader, optim, optim_meta, sched, swa_model=None, swa_scheduler=None):
    prev_epoch = 0
    import copy
    if args.load_model != '':
        loaded_path = os.path.split(args.load_model)[0]
        ...
        for i_ep in range(args.epochs):
            ...
            for idx, (data, targets) in enumerate(train_loader):
                ...
                with higher.innerloop_ctx(model, optim,
                    ...
                    # mu_y_out, sigma_y_out = model.forward(data)
                    mu_y_out, sigma_y_out = theta_pi.forward(data)
                    # labels = nn.functional.one_hot(targets, list(model.children())[-2].out_features)
                    labels = nn.functional.one_hot(targets, list(theta_pi.children())[-2].out_features)
                    # loss = model.batch_loss(mu_y_out, sigma_y_out, labels)
                    loss = theta_pi.batch_loss(mu_y_out, sigma_y_out, labels)
                    ...
                    # diff_optim.step(loss)
                    # File "/home/khanhi83/anaconda3/envs/PY3/lib/python3.7/site-packages/higher/optim.py", line 233, in step
                    # allow_unused=True  # boo
                    # File "/home/khanhi83/anaconda3/envs/PY3/lib/python3.7/site-packages/torch/autograd/__init__.py", line 204, in grad
                    # inputs, allow_unused)
                    ...
                    # model.load_state_dict(theta_pi.state_dict())
                    loss.backward()
                    optim_meta.step()
